...
function buildReplayUserMessageEvent(params) {
...
function fallbackEnhancedPrompt(params) {
...
async function collectLlmText(params) {
    ...
    for await (const delta of llm.streamChatCompletions({
        ...
        if (delta.content)
            text += stripThinkTags(delta.content);
    }
    return text.trim() || null;
}
function buildProseInstruction(option, command) {
    if (command?.trim())
        return `Follow this editing instruction: ${command.trim()}`;
    if (option === "improve")
        return "Improve the writing while preserving the original meaning.";
    if (option === "shorter")
        return "Make the text shorter while preserving the key points.";
    if (option === "longer")
        return "Expand the text with useful detail while preserving the original direction.";
    if (option === "continue")
...
app.post("/api/prompt/enhance", async (request, reply) => {
    try {
        ...
        const enhanced = (await collectLlmText({
            system: "You improve academic research prompts. Return only the enhanced prompt text, no markdown fences and no JSON.",
...
app.post("/api/report/evaluate", async (request, reply) => {
    try {
        ...
        if (parsed.use_llm) {
            const llmText = await collectLlmText({
                system: "You are a strict academic report evaluator. Output ONLY valid JSON matching the requested schema.",
                user: "Evaluate the report from 0 to 10. Return JSON with scores {factual_accuracy, completeness, coherence, relevance, citation_quality, writing_quality}, overall_score, weighted_score, strengths, weaknesses, suggestions.\n\n" +
...
app.options("/api/prose/generate", async (request, reply) => {
    const origin = typeof request.headers.origin === "string" ? request.headers.origin : undefined;
    applyCorsHeaders(origin, reply);
    reply.raw.statusCode = 204;
    reply.raw.end();
...
app.post("/api/prose/generate", async (request, reply) => {
    const origin = typeof request.headers.origin === "string" ? request.headers.origin : undefined;
    applyCorsHeaders(origin, reply);
    setupSse(reply);
    try {
        ...
        for await (const delta of llm.streamChatCompletions({
            ...
            if (cleaned)
                writeSseEvent(reply, { data: cleaned });
        }
    }
    catch (e) {
...
app.post("/api/mcp/server/metadata", async (request, reply) => {
    if (!env.ENABLE_MCP_SERVER_CONFIGURATION) {
        reply
            .code(403)
            .send(toDetailError("MCP server configuration is disabled. Set ENABLE_MCP_SERVER_CONFIGURATION=true to enable MCP features."));
        return reply;
    }
    try {
...
app.get("/api/rag/resources", async (request, reply) => {
    await ensureRagDir();
    const query = typeof request.query === "object" && request.query && "query" in request.query
...
app.get("/api/replays/:threadId/latest", async (request, reply) => {
    const params = request.params;
...
app.get("/api/replays/:threadId/:runId", async (request, reply) => {
    const params = request.params;
    const threadId = String(params.threadId ?? "");
...
app.get("/api/replays/:threadId", async (request, reply) => {
    const params = request.params;
    const threadId = String(params.threadId ?? "");
    if (!threadId) {
        reply.code(400).send(toDetailError("Missing thread id"));
...
app.get("/api/traces/:threadId/latest", async (request, reply) => {
    const params = request.params;
    const threadId = String(params.threadId ?? "");
    if (!threadId) {
        reply.code(400).send(toDetailError("Missing thread id"));
...
app.get("/api/traces/:threadId/:runId", async (request, reply) => {
    const params = request.params;
    const threadId = String(params.threadId ?? "");
    const runId = String(params.runId ?? "");
    if (!threadId || !runId) {
...
app.get("/api/traces/:threadId", async (request, reply) => {
    const params = request.params;
    const threadId = String(params.threadId ?? "");
    if (!threadId) {
        reply.code(400).send(toDetailError("Missing thread id"));
