...
function plainChatFallback(locale) {
...
function plainChatSystemPrompt(locale) {
    return [
...
async function* runPlainChatResponse(params) {
    ...
    try {
        ...
        const resourceNotice = request.resources.length
            ? [
                {
                    role: "system",
                    content: "The user attached resources, but normal chat mode does not retrieve or inspect resource contents. If the request depends on those materials, ask the user to enable Research Mode.",
                },
            ]
            : [];
