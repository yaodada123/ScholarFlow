...
export function evaluateReportDeterministic(content: string): EvaluationResult {
  ...
  const urls = new Set(
    Array.from(content.matchAll(/https?:\/\/[^\s)\]]+/g), (match) => match[0].replace(/[.,;:]+$/, "")),
  );
...
export function parseLlmEvaluation(text: string): LlmEvaluation | null {
...
function matchesSection(heading: string, section: string): boolean {
...
function computeScore(metrics: EvaluationMetrics): number {
...
function buildSummary(metrics: EvaluationMetrics, score: number): string {
