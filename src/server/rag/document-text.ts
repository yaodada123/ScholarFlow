...
export function sanitizeRagFilename(input: string): string {
...
export function buildLocalResource(filename: string): Resource {
  const safeName = sanitizeRagFilename(filename);
  return { uri: `rag://local/${safeName}`, title: safeName, description: "" };
}

export function localFilenameFromUri(uri: string): string | null {
...
export async function extractTextFromRagFile(
  filePath: string,
  filename: string,
): Promise<{ text: string; sourceType: "text" | "pdf"; pageCount?: number }> {
  const safeName = sanitizeRagFilename(filename);
  const ext = path.extname(safeName).toLowerCase();
  if (ext === ".md" || ext === ".txt") {
    const content = await readFile(filePath, { encoding: "utf8" });
    const text = normalizeExtractedText(content);
    if (!text) throw new RagTextExtractionError("Could not extract text from the uploaded file.");
  ...
  if (ext === ".pdf") {
    ...
    if (!ocrConfig.enabled) {
      if (textExtractionError) throw textExtractionError;
      throw new RagTextExtractionError(
        "Could not extract selectable text from PDF. Scanned/image-only PDFs require PDF_OCR_ENABLED=true.",
      );
    }
