...
function headersFromConfig(config: McpServerConfig): Record<string, string> | undefined {
...
async function withClient<T>(config: McpServerConfig, fn: (client: Client) => Promise<T>): Promise<T> {
  const client = new Client(CLIENT_INFO, { capabilities: {} });
  const transport = createTransport(config);
  try {
    await client.connect(transport as Transport, { timeout: MCP_CONNECT_TIMEOUT_MS });
...
export async function listMcpServerTools(config: McpServerConfig): Promise<McpToolMetadata[]> {
...
function serializeToolResult(result: unknown): McpToolCallResult {
  if (!result || typeof result !== "object") return { content: stringifyLimited(result) };
...
function serializeContentItem(item: unknown): string {
  if (!item || typeof item !== "object") return stringifyLimited(item);
  const raw = item as Record<string, unknown>;
  if (raw.type === "text" && typeof raw.text === "string") return limitText(raw.text);
  if (raw.type === "image") return `[image: ${typeof raw.mimeType === "string" ? raw.mimeType : "unknown mime"}]`;
  if (raw.type === "audio") return `[audio: ${typeof raw.mimeType === "string" ? raw.mimeType : "unknown mime"}]`;
